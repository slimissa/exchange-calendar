...
class TestXQSEIslamicNewYearAndMawlid:
    ...
    v2.1.2 correction: Qatar (along with Saudi Arabia) does not
    observe Mawlid al-Nabi as an official public holiday at all --
    confirmed by multiple independent sources (OfficeHolidays,
    qppstudio's exhaustive country list explicitly excludes only
    Saudi Arabia and Qatar among Muslim-majority countries) and by
    Qatar's own religious-affairs position (Salafi/Wahhabi tradition
    treats the celebration as a religious innovation). All Mawlid
    entries removed. The Islamic New Year 2025 entry that had briefly
    been marked confirmed was reverted back to predicted -- what was
    actually confirmed was a Hijri calendar-transition announcement,
    not evidence that Qatar Exchange closes for it as a trading
    holiday; Qatar's known official holiday set is narrower than its
    civil/religious calendar."""
    ...
    def test_mawlid_absent(self, explicit_dates):
        """Qatar does not observe Mawlid al-Nabi as an official public
        holiday (Salafi/Wahhabi tradition; confirmed alongside Saudi
        Arabia by multiple independent sources). No exchange entry
        should exist for any year -- previously predicted entries for
        2025-2029 were removed during v2.1.2 reconciliation."""
        for entry in explicit_dates.values():
            assert "Prophet" not in entry["name"], \
                f"Unexpected Mawlid entry: {entry['date']} ({entry['name']})"