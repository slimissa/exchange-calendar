...
class TestXCAIProperties:
    ...
    def test_generation_range(self, xcai):
        ...
        # Shortened from 2029-12-31 per H1-fix: XCAI's Islamic-holiday
        ...
        assert xcai["generation_range"] == ["2025-01-01", "2029-07-24"]
...
class TestXCAIEidAlFitr:
    def test_eid_al_fitr_2025(self, explicit_dates):
        """Eid al-Fitr 2025 — Egypt's Dar al-Ifta confirmed March 31,
        one day after Saudi's Umm al-Qura date of March 30 (C6:
        Egypt uses its own moon-sighting committee and can legitimately
        diverge from Saudi by ±1 day). The holiday runs March 31 -
        April 2."""
...
class TestXCAIIslamicHolidays:
    ...
    def test_islamic_holidays_contain_predicted(self, explicit_dates):
        """The (predicted) name suffix and the predicted field must
        agree -- entries get reconciled over time (see
        tools/verify_predicted_dates.py), so this checks consistency
        rather than asserting these holidays are forever predicted."""
        islamic_names = ["Islamic New Year", "Prophet"]
        for entry in explicit_dates.values():
            if any(name in entry["name"] for name in islamic_names):
                name_says_predicted = "(predicted)" in entry["name"]
                field_says_predicted = entry.get("predicted") is True
                assert name_says_predicted == field_says_predicted, (
                    f"{entry['name']!r}: name suffix and predicted "
                    f"field disagree ({name_says_predicted} vs "
                    f"{field_says_predicted})"
                )
...
class TestXCAIStructure:
    ...
    def test_source_url_consistency(self, explicit_dates):
        """EGX's own trading-calendar page is the source for fixed
        national holidays. Islamic holidays are sourced from Dar
        al-Ifta directly (C6) since EGX's page doesn't itemize
