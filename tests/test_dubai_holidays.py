...
class TestXDFMIslamicHolidays:
    ...
    MAWLID = {
        2025: "2025-09-05", 2026: "2026-08-25",
    ...
    def test_all_islamic_entries_marked_predicted(self, explicit_dates):
        """Name suffix and predicted field must agree; 2025's Islamic New
        Year and Eid al-Fitr entries were reconciled (v2.1.2) and no
        longer carry the (predicted) suffix -- see
        ...
        for date_str, entry in explicit_dates.items():
            if any(k in entry["name"] for k in islamic_keywords):
                ...
                assert name_says_predicted == field_says_predicted, \
                    f"{date_str} ({entry['name']}): name suffix and predicted field disagree"
