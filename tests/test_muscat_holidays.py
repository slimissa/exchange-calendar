...
class TestXMUSIslamicHolidays:
    ...
    def test_prophets_birthday_2025(self, explicit_dates):
        """Prophet's Birthday 2025 — confirmed September 7 (Oman Ministry
        of Labour, 14 Rabi' al-Awwal 1447 AH). Corrected from an
        erroneous September 4 during Mawlid-cluster reconciliation
        (v2.1.2) -- Oman's date diverged from the Gulf-wide September 4
        the whole cluster had shipped with."""
        assert "2025-09-07" in explicit_dates
        assert "Prophet" in explicit_dates["2025-09-07"]["name"]
        assert explicit_dates["2025-09-07"]["name"] == "Prophet's Birthday"
