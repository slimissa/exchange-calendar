...
def validate_schema(exchange: dict, schema: dict, filename: str) -> list:
    ...
    if not HAS_JSONSCHEMA:
        ...
        # IMPORTANT: this fallback does NOT check regex patterns (code
        # format, timezone format, hours format, etc) -- only presence of
        # required fields. A file with all required fields present but a
        # malformed value (e.g. an invalid MIC code) will NOT be caught here.
        # This is a known, real gap, not a hypothetical one -- surface it
        # loudly rather than let validation look complete when it isn't.
        print(
            "WARNING: jsonschema package not installed -- schema pattern "
            "validation (code/timezone/hours format, etc) is being SKIPPED. "
            "Only required-field presence is checked. Run "
            "`pip install jsonschema` (now in tools/requirements.txt) before "
            "trusting a clean validation result.",
            file=sys.stderr,
        )
...
def check_past_due_predictions(exchange: dict, filename: str) -> list:
    ...
    KNOWN_PAST_DUE = {
        "XTAD", "XDFM", "XCAS", "XKUW", "XKAR",
        "XDHA", "XBAH", "XTUN", "XMUS", "XCAI", "XNSA", "XNBO",
    }
