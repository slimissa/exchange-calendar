...
Note: run `python3 tools/verify_checksums.py` to check the exchange
data files, `schema.json`, and `tools/*.py` against the committed
`checksums.json` manifest (generated by `tools/generate_checksums.py`).
This covers source data and tooling, not `calendar.json` itself
(a build artifact -- rebuild and diff it, or checksum its declared
inputs instead). Exit code 0 means every listed file matched.
...
`tools/generate_checksums.py` writes a SHA-256 manifest
(`checksums.json`) covering all 74 `exchanges/*.json` files,
`schema.json`, and `tools/*.py`. `tools/verify_checksums.py` checks
the working tree against that manifest and exits non-zero on any
mismatch or missing file -- suitable for a CI step or a pre-publish
check. Re-run `generate_checksums.py` and commit the updated
`checksums.json` whenever any covered file legitimately changes;
a stale manifest will fail verification the same as tampering would.
